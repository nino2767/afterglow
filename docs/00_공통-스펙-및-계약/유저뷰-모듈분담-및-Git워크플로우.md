...
> 버전: v1.1 | 작성일: 2026.07.29 | 상태: 📋 정식 확정 (모노레포 개편 반영)
> 근거: 스핀오프 유저뷰 IA v1.0, 화면별 상세기획서 7종(v0.1), SSOT v1.3, 어드민 모노레포 통합 (`admin-view/`)
> 담당: **J = 본전시(main) + 계정모드(account)** · **Sol = 스핀오프(spinoff)**
> ⚠️ 저장소 구조: 단일 모노레포 (`user-view/` 및 `admin-view/`, 기획문서 `docs/00~07`)
