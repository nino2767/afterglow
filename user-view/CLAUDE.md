...
- **현재 상황**: `docs/00_공통-스펙-및-계약/00_데이터-계약-SSOT_v1.2.md`와 `docs/02_관람객-유저서비스/00_IA-및-인벤토리/데이터-수집-방식-및-정보.md`의 감정 모델(8축, 세그먼트) 정의 충돌이 해결될 때까지 **데이터 레이어 개발은 일시 보류**합니다.
- **적용 규칙**:
...
- `src/app/(main)/`: 본전시 가이드 모드 화면군 (S1~S4)
- `src/app/(spinoff)/`: 스핀오프 가이드 모드 화면군 (S5~S6)
  - `src/app/(spinoff)/spinoff/landing/`: 스핀오프 랜딩 (화면5, URL: `/spinoff/landing`)
  - `src/app/(spinoff)/popup-curator/`: 팝업 큐레이터 (화면6, URL: `/popup-curator`)
- `src/app/(account)/`: 계정 모드 화면군 (S0, S7~S10)
- `src/components/`: 공통/모듈별 컴포넌트 (`shared/`, `main/`, `spinoff/`, `account/`)
- `src/lib/`: 공통/모듈별 로직 및 SSOT (`shared/`, `main/`, `spinoff/`)
